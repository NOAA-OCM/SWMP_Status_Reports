...
for(i in 1:length(ls_par)) {
  ...
  for(j in 1:length(par_inst)) {
    ...
    if(!is.na(par_threshold_val[j])) {
      ...
      write.csv(threshold_id_tbl, file = threshold_id_ttl, row.names = F)
    ...
    trnd_lab <- ifelse(dat[c('datetimestamp', par[j])] %>% .[complete.cases(.), ] %>% .[, 1] %>% year %>% unique() %>% length == 1, F, T)
...
if(length(par_cumulative) > 0) {
  for(i in 1:length(ls_par)) {
    ...
    converted_par <- ifelse(convert_temp && (par[j] == 'atemp' | par[j] == 'totprcp'), T, F)
    ...
    for(j in 1:length(par_cumulative)) {
      ...
      # Determine plot conversion
      converted_par <- ifelse(convert_temp && par_cumulative[j] == 'totprcp', T, F)
