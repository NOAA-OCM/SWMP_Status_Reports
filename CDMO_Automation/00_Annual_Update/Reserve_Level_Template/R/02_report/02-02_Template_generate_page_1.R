...
#                        location = ph_location(left = 0.17, top = 0.42, 
#                                               width = 8.3, height = 0.51))
# doc <- doc %>% ph_add_par(id_chr = "3") %>% 
#   ph_add_text(str = txt_rpt_ttl, type = 'body', id_chr = "3", 
#               style = format_ttl)
...
# doc <- doc %>% ph_with(value = empty_content(), 
#                        location = ph_location(left = 0.18, top = 0.91, 
#                                               width = 4.85, height = 0.71))
...
doc <- doc %>% ph_add_par(id_chr = "6") %>% 
  ph_add_text(str = txt_nerr_desc, type = 'body', 
              id_chr = "6", style = format_nerr_desc)

# Add NERR website to description
doc <- doc %>% ph_with(value = empty_content(), 
                       location = ph_location(left = 0.19, top = 8.69, 
                                              width = 2.94, height = 0.31))
doc <- doc %>% ph_add_par(id_chr = "7") %>% 
  ph_add_text(str = txt_nerr_website, type = 'body', 
              id_chr = "7", style = format_nerr_web)
...
doc <- doc %>% ph_add_par(id_chr = "8") %>%
  ph_add_text(str = paste(target_year, txt_highlight_ttl), type = 'body', 
              id_chr = "8", style = format_highlight_ttl)
...
doc <- doc %>% ph_add_par(id_chr = "9") %>%
  ph_add_text(str = txt_highlight_1, type = 'body', 
              id_chr = "9", style = format_highlight)

doc <- doc %>% ph_with(value = empty_content(), 
                       location = ph_location(left = 4.32, top = 6.94, 
                                              width = 3.6, height = 0.62))
doc <- doc %>% ph_add_par(id_chr = "10") %>%
  ph_add_text(str = txt_highlight_2, type = 'body', 
              id_chr = "10", style = format_highlight)

doc <- doc %>% ph_with(value = empty_content(), 
                       location = ph_location(left = 4.32, top = 7.99, 
                                              width = 3.6, height = 0.62))
doc <- doc %>% ph_add_par(id_chr = "11") %>%
  ph_add_text(str = txt_highlight_3, type = 'body', 
              id_chr = "11", style = format_highlight)

doc <- doc %>% ph_with(value = empty_content(), 
                       location = ph_location(left = 4.32, top = 8.98, 
                                              width = 3.6, height = 0.62))
doc <- doc %>% ph_add_par(id_chr = "12") %>%
  ph_add_text(str = txt_highlight_4, type = 'body', 
              id_chr = "12", style = format_highlight)
