...
$old_zip_dir = "D:\SWMP\2019_Version1" # Absolute path for ZIPPED old site directories
$data_path = "D:\SWMP\data2020"    # Absolute path to data directory, where all
...
$previous_year_path_R = $previous_year_path -replace "\\", "/" 
$old_zip_dir_R = $old_zip_dir -replace "\\", "/"
...
if (Test-Path -Path $previous_year_path) {
    Move-Item -Path $previous_year_path  "$previous_year_path.Backup" -Force  #Backup existing distribution files
}
New-Item -ItemType Directory -Force -Path $previous_year_path
...
$tmpfile = "$($root)\$($Rmaster)"
[System.IO.File]::WriteAllLines($tmpfile, $Rlines2, [System.Text.UTF8Encoding]($False))
